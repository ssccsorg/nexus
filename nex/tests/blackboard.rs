...
use nexus_storage_composite::CompositeBlackboard;
...
fn bb_with_facts() -> CompositeBlackboard {
    let bb = CompositeBlackboard::new();
    for i in 0..5 {
        let fact = Fact {
...
fn test_cursor_survives_snapshot_roundtrip() {
    ...
    let restored = CompositeBlackboard::from_snapshot(snap);
    assert_eq!(
...
fn test_flush_after_restore_continues_from_cursor() {
    ...
    let snap = bb.to_snapshot();
    let mut restored = CompositeBlackboard::from_snapshot(snap);
...
fn test_cursor_independent_of_graph_mutations() {
    ...
    let mut bb = CompositeBlackboard::new();
    let fact = Fact {
        id: FihHash("f1".into()),
        origin: "test".into(),
        content: "test".into(),
...
fn test_storage_snapshot_roundtrip() {
    ...
    // Reconstruct
    let restored = CompositeBlackboard::from_snapshot(snapshot);

    // Verify graph data
    let state = <CompositeBlackboard as nexus_model::StorageRead>::read_state(&restored);
