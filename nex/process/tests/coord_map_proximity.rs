// ── Phase 2 (#166): search.json scenario over the materialized CoordMap ──
//
// Documents are indexed into a coordinate space (each axis is a
// vocabulary term, the coordinate is the term count), and a CoordCubeMap
...
/// Index the documents into `map` at their coordinate paths. The value is
/// the document id, the opaque record at the codec seam.
fn index_docs(map: &mut CoordMapStore<6>, docs: &[Doc]) {
    for doc in docs {
        let previous = map.put_path(&doc_path(doc), doc.id.as_bytes()).unwrap();
...
fn proximity_search_retrieves_matching_entries() {
    ...
    {
        let mut map = CoordMapStore::<6>::new(Box::new(FileOrigin::open(&path).unwrap()), 256);
        index_docs(&mut map, DOCS);
        assert_eq!(map.len(), DOCS.len());

        // Radius 1 from the storage center finds only the center doc.
        let r1 = map.proximity::<6, 1>(&storage_center(), 1);
        ...
        let r2 = map.proximity::<6, 1>(&storage_center(), 2);
        ...
        let bb = map.bounding_box_range(&ranges);
...
fn proximity_search_persists_across_file_reopen() {
    ...
    {
        let map =
            CoordMapStore::<6>::load(Box::new(FileOrigin::open(&path).unwrap()), 256).unwrap();
        assert_eq!(map.len(), DOCS.len());
        let results = map.proximity::<6, 1>(&storage_center(), 2);
        assert_eq!(
            collect_ids(results),
...
fn proximity_search_over_mapped_file_origin() {
    ...
    {
        let mut map =
            CoordMapStore::<6>::new(Box::new(MappedFileOrigin::open(&path).unwrap()), 256);
        index_docs(&mut map, DOCS);
        map.flush().unwrap();
    }
    {
        let map = CoordMapStore::<6>::load(Box::new(MappedFileOrigin::open(&path).unwrap()), 256)
            .unwrap();
        let results = map.proximity::<6, 1>(&storage_center(), 2);
        assert_eq!(
            collect_ids(results),
