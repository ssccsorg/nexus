...
fn scenario_peer_review_challenge() {
    ...
    if let Some(cf) = contradiction_fact {
        let intent = Intent {
            id: CoordId::resolve(&format!("{} peer-review intent", cf.id)),
            from_facts: vec![cf.id],
...
fn scenario_multi_agent_collaboration() {
    ...
    for gf in &gap_facts {
        let intent = Intent {
            id: CoordId::resolve(&format!("{} alpha intent", gf.id)),
    ...
    for gf in &compiler_gaps {
        let intent = Intent {
            id: CoordId::resolve(&format!("{} beta intent", gf.id)),
            from_facts: vec![gf.id],
    ...
    for cf in &contradiction_facts {
        let intent = Intent {
            id: CoordId::resolve(&format!("{} gamma intent", cf.id)),
            from_facts: vec![cf.id],
