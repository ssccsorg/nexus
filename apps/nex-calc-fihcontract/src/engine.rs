...
pub struct CalcEngine {
    ...
    pub contract: FihContract,
}

impl CalcEngine {
    pub fn new() -> Self {
        let contract = FihContract::new();
        contract.gate.register_schema(NUMBER_MIME, NUMBER_MIME.as_bytes());
        Self {
    ...
    pub async fn put(&self, value: i64) -> FihHash {
        ...
        if let Err(e) = self.contract.gate.admit(NUMBER_MIME, &data) {
    ...
    pub async fn resolve(&self, intent_id: &FihHash) -> Result<ResolvedIntent, CalcError> {
        ...
        if let Err(e) = self.contract.hints.check_numeric(raw_result) {
        ...
        let ts = nanos();
        self.contract.evidence.append(&intent_id.to_string(), "intent:conclude", ts);
...
mod tests {
    ...
    async fn test_add_intent_and_resolve() {
        ...
        assert!(engine.contract.evidence.len() > 0);
    }
