...
/// Load state from the full checkpoint, then rebuild the cache from io
/// so records submitted after the last checkpoint are included.
async fn restore_from_snapshot(s: &AppStorage) -> Result<bool, String> {
    let Some(bytes) = s.io.read(SNAPSHOT_KEY).await? else {
        return Ok(false);
    };
    let snapshot: Snapshot =
...
async fn fetch_ssccs_docs(s: &AppStorage) -> (usize, Vec<String>) {
    ...
    for url in &urls {
        ...
        if let Some(cached) = cache.docs.get(url) {
            new_cache.insert(url.clone(), cached.clone());
            continue;
        }
            ...
            Ok(text) => match ingest_document(s, &text, &origin).await {
                Ok(_) => {
                    ...
                    new_cache.insert(
                        url.clone(),
                        DocEntry {
                            content_hash: compute_sha256(text.as_bytes()),
                        },
                    );
                }
    ...
    cache.docs = new_cache;
    write_cache(&cache).await;
    if total > 0 {
        let _ = s.rebuild_cache().await;
        s.rebuild_semantic().await.ok();
        if let Err(e) = write_checkpoint(s).await {
...
async fn fetch_url(url: &str) -> Result<String, String> {
    ...
    let res: spin_sdk::http::Response = spin_sdk::http::send(req)
        .await
...
async fn handler(req: Request<Vec<u8>>) -> anyhow::Result<impl IntoResponse> {
    ...
    let resp = match (method, path.as_str()) {
        ...
        (Method::POST, "/ingest") => {
            let params: IngestParams = match serde_json::from_slice(&body) {
                Ok(p) => p,
                Err(e) => return err_json(400, "invalid_json", format!("{e}")),
            };
        ...
        (Method::GET, "/search") => {
            ...
            let top_k = query_str
                .split('&')
                .find_map(|p| p.strip_prefix("top_k="))
                .and_then(|v| v.parse().ok())
                .unwrap_or(10);
            if q.is_empty() {
                return err_json(400, "validation_error", "missing q".into());
            }
        ...
        (Method::GET | Method::POST, "/sync-docs") => {
            ...
            ok_json(
                serde_json::json!({"status": if errors.is_empty() { "ok" } else { "partial" }, "ingested": total, "errors": errors}),
            )
        }
        (Method::GET, "/state") => {
        ...
        (Method::POST, "/fact") => {
            let params: FactParams = match serde_json::from_slice(&body) {
                Ok(p) => p,
                Err(e) => return err_json(400, "invalid_json", format!("{e}")),
            };
            let id = params
            ...
            let hash = match get_storage().submit_fact(&fact).await {
                Ok(h) => h,
                Err(e) => return err_json(500, "fact_error", format!("{e:?}")),
            };
            if let Err(e) = get_storage().flush_pending().await {
                return err_json(500, "flush_error", e);
            }
            ok_json(serde_json::json!({"id": hash.to_string()}))
        }
        (Method::POST, "/intent") => {
            let params: IntentParams = match serde_json::from_slice(&body) {
                Ok(p) => p,
                Err(e) => return err_json(400, "invalid_json", format!("{e}")),
            };
            let id = params
                .id
                .unwrap_or_else(|| format!("intent_{}", timestamp_id()));
            let from_facts: Vec<CoordId> = params
                .from
                .as_deref()
                .unwrap_or("")
                .split(',')
            ...
            if from_facts.is_empty() {
                return err_json(
                    400,
                    "validation_error",
                    "intent needs at least one fact".into(),
                );
            }
            ...
            let hash = match get_storage().submit_intent(&intent).await {
                Ok(h) => h,
                Err(e) => return err_json(500, "intent_error", format!("{e:?}")),
            };
            ok_json(serde_json::json!({"id": hash.to_string()}))
        }
        (Method::POST, p) if p.starts_with("/intent/") => {
            let s = p.trim_start_matches("/intent/");
            let params: ClaimParams = match serde_json::from_slice(&body) {
                Ok(p) => p,
                Err(e) => return err_json(400, "invalid_json", format!("{e}")),
            };
            if let Some(id) = s.strip_suffix("/claim") {
                ...
                match get_storage().heartbeat(id, &params.agent).await {
                    Ok(_) => ok_json(serde_json::json!({"status": "ok"})),
                    Err(e) => err_json(500, "heartbeat_error", format!("{e:?}")),
                }
            } else if let Some(id) = s.strip_suffix("/release") {
                match get_storage().release_intent(id, &params.agent).await {
                    Ok(_) => ok_json(serde_json::json!({"status": "released"})),
                    Err(e) => err_json(500, "release_error", format!("{e:?}")),
                }
            } else if let Some(id) = s.strip_suffix("/conclude") {
                let cp: ConcludeParams = match serde_json::from_slice(&body) {
                    Ok(p) => p,
                    Err(e) => return err_json(400, "invalid_json", format!("{e}")),
                };
                match get_storage().conclude_intent(id, &cp.result).await {
                    Ok(f) => ok_json(
                        serde_json::json!({"status": "concluded", "fact_id": f.id.to_string()}),
                    ),
                    Err(e) => err_json(500, "conclude_error", format!("{e:?}")),
                }
            } else {
                err_json(404, "not_found", format!("no handler for {p}"))
            }
        }
        (Method::POST, "/hint") => {
            let params: HintParams = match serde_json::from_slice(&body) {
                Ok(p) => p,
                Err(e) => return err_json(400, "invalid_json", format!("{e}")),
            };
            let id = params
            ...
            if let Err(e) = get_storage().submit_hint(&hint).await {
                return err_json(500, "hint_error", format!("{e:?}"));
            }
        ...
        (Method::GET | Method::POST, "/rebuild") => {
            if let Err(e) = get_storage().rebuild_cache().await {
                return err_json(500, "rebuild_error", e);
            }
            if let Err(e) = get_storage().rebuild_semantic().await {
                return err_json(500, "rebuild_semantic_error", e);
            }
            ok_json(serde_json::json!({"status": "ok"}))
        }
        // ── Full checkpoint (SurrealKV snapshot pattern) ────────────
        (Method::POST, "/checkpoint") => match write_checkpoint(get_storage()).await {
        ...
        _ => err_json(
            404,
            "not_found",
            format!("no route for {method_str} {path}"),
        ),
    };
    resp
}
