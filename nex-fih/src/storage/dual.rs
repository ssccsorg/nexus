...
use crate::storage::scan::PartitionData;
...
impl ScanCapable for DualStorage {
    fn scan_partition(&self, partition: &str) -> Result<PartitionData, String> {
        ...
        cold_data.facts.sort_by_key(|a| a.id.to_string());
        cold_data.intents.sort_by_key(|a| a.id.to_string());
        cold_data.hints.sort_by_key(|a| a.id.to_string());
