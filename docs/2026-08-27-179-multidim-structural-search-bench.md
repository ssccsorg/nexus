...
|---|---|---|---|---|
| origin + creator + time, wide | 100k | 1.44 ms | 257 µs | 6x |
| origin + creator + time, narrow | 100k | 940 µs | 37.4 µs | 25x |
| origin + creator + time, wide | 1m | 25.3 ms | 5.00 ms | 5x |
| origin + creator + time, narrow | 1m | 18.6 ms | 398 µs | 47x |
| creator only, wide | 1m | 110 ms | 720 ms | structural slower |

The structural path wins for time-bounded origin-plus-creator filters,
and the gap widens with scale (narrow: 25x at 100k, 47x at 1m). The
crossover sits below 100k. The creator-only case is the axis-order
limitation: without origin fixed, the prefix cannot prune creator, the
path does a full-tree walk, and the scan is cheaper.
...
faster on the structural path by 2.4x at phase 1 and 2.9x at phase 3.
The creator-only query on real data repeats the axis-order limitation:
the structural path takes 3.4 to 5.1 ms against 8.5 to 14 µs for the
scan.
...
The marginal record cost is about 901 bytes per fact (record maps plus
record storage). The structural intercept, about 264 MB for 1000 dense
leaves, is the fixed spatial-index cost: the same axis-combo space costs
the same memory regardless of whether it holds 100k or 1m records. The
claim holds within the axis-combo boundary.
...
`iter_prefix` at depth 5 is sub-microsecond, so the day-walk cost is
O(window days) prefix navigations, and index maintenance stays an
id-set push per write. Concrete chton and syntagma improvements will be
