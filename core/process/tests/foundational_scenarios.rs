...
use nexus_graph::{
    Blackboard, EvictCapable, Fact, FihHash, Intent, Snapshottable, create_blackboard,
};
...
fn do_tick(sched: &mut Scheduler<impl Blackboard + EvictCapable + Snapshottable>) -> usize {
    sched.tick().expect("tick")
}

fn facts_by_creator<'a>(state: &'a nexus_graph::BoardState, creator: &str) -> Vec<&'a Fact> {
    state
        .facts
        .iter()
...
fn scenario_foundational_consistency_audit() {
    ...
    let field_tensions: Vec<_> = contradictions
        .iter()
        .filter(|f| f.content.get("topic").and_then(|v| v.as_str()) == Some("field-definition"))
        .collect();
    assert!(
    ...
    let segment_tensions: Vec<_> = contradictions
        .iter()
        .filter(|f| f.content.get("topic").and_then(|v| v.as_str()) == Some("segment-definition"))
        .collect();
    assert!(
...
fn scenario_formal_revision_of_philosophy() {
    ...
    let field_contradiction = state2.facts.iter().find(|f| {
        f.creator == "contradiction-detector"
            && f.content.get("topic").and_then(|v| v.as_str()) == Some("field-definition")
    });
