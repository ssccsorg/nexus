...
fn stamp(fact: &Fact, ts: &str) -> String {
    ...
    struct S<'a, T> { submitted_at: &'a str, data: &'a T }
    serde_json::to_string(&S { submitted_at: ts, data: fact }).unwrap()
}
...
fn test_flush_updates_cursor_on_commit_kv() {
    ...
    // Submit data
    s.kv().set("cqrs-test:fact:f1", &stamp(&test_fact("f1", "100"), "100")).unwrap();

    let cursor = FlushCursor { last_flushed_at: String::new(), partition: "p".into() };
    ...
    let new_cursor = s.read_cursor().unwrap().expect("cursor should exist after flush");
...
fn test_flush_writes_to_commit_blob() {
    ...
    assert!(!commit_blobs.is_empty(), "commit blob should have flush output");
}
...
fn test_cursor_tracks_flush_boundary() {
    ...
    s.kv().set("cqrs-test:fact:old", &stamp(&test_fact("old", "100"), "100")).unwrap();
    s.flush_since(&FlushCursor { last_flushed_at: String::new(), partition: "p".into() }).unwrap();
    let cursor1 = s.read_cursor().unwrap().unwrap().last_flushed_at.clone();

    // Phase 2: submit new data with ts=200
    s.kv().set("cqrs-test:fact:new", &stamp(&test_fact("new", "200"), "200")).unwrap();
    s.flush_since(&FlushCursor { last_flushed_at: cursor1.clone(), partition: "p".into() }).unwrap();
    ...
    let threads: Vec<_> = (0..4).map(|i| {
        ...
        thread::spawn(move || {
            ...
            for j in 0..10 {
                ...
                s.kv().set(&key, &stamp(&test_fact(&id, "100"), "100")).unwrap();
            }
        })
    }).collect();
...
fn test_concurrent_read_during_write() {
    ...
    // Pre-populate
    for i in 0..10 {
        s.kv().set(&format!("cqrs-test:fact:f{i}"), &stamp(&test_fact(&format!("f{i}"), "100"), "100")).unwrap();
    }
    ...
    let writer = thread::spawn(move || {
        for i in 10..20 {
            sw.kv().set(&format!("cqrs-test:fact:f{i}"), &stamp(&test_fact(&format!("f{i}"), "200"), "200")).unwrap();
        }
    });
    ...
    assert!(final_count >= 10, "reader should see at least pre-populated data");
...
fn test_flush_then_read_cursor_via_commit_kv() {
    ...
    assert_eq!(saved.last_flushed_at, new_cursor_ts, "commit_kv cursor should match flush result");
}
...
fn test_incremental_flush_respects_cursor() {
    ...
    // First batch
    s.kv().set("cqrs-test:fact:a", &stamp(&test_fact("a", "100"), "100")).unwrap();
    s.kv().set("cqrs-test:fact:b", &stamp(&test_fact("b", "100"), "100")).unwrap();

    let r1 = s.flush_since(&FlushCursor { last_flushed_at: String::new(), partition: "p".into() }).unwrap();
    assert_eq!(r1.records_flushed, 2, "first flush: 2 facts");
    assert!(!r1.new_cursor.last_flushed_at.is_empty(), "cursor should be set");
    let c1 = r1.new_cursor.last_flushed_at.clone();

    // Second batch — same timestamp, should be excluded by cursor
    s.kv().set("cqrs-test:fact:c", &stamp(&test_fact("c", "100"), "100")).unwrap();
    let r2 = s.flush_since(&FlushCursor { last_flushed_at: c1.clone(), partition: "p".into() }).unwrap();
    assert_eq!(r2.records_flushed, 0, "second flush: timestamp not newer than cursor");

    // cursor was updated (flush_since always writes new cursor, even with 0 records)
    let saved = s.read_cursor().unwrap().unwrap();
    assert!(!saved.last_flushed_at.is_empty(), "cursor should exist after flush");
}
