...
struct Fact {
    id: String, origin: String, tags: Vec<String>,
    title: String, href: String, text_len: usize,
    r2_key: String, creator: String, created_at: String,
}

#[derive(Debug, Serialize, Deserialize, Clone)]
struct Intent {
    id: String, from_facts: Vec<String>, description: String,
    creator: String, worker: Option<String>,
    to_fact_id: Option<String>,
    created_at: Option<String>, concluded_at: Option<String>,
}

#[derive(Debug, Serialize, Deserialize, Clone)]
struct Hint {
    id: String, content: String, creator: String,
}
...
fn fact_key(id: &str) -> String { format!("fact:{}", encode_id(id)) }
fn intent_key(id: &str) -> String { format!("intent:{}", encode_id(id)) }
fn hint_key(id: &str) -> String { format!("hint:{}", encode_id(id)) }
fn tag_index_key(tag: &str) -> String { format!("tag:{}", encode_id(tag)) }
fn all_index(kind: &str) -> String { format!("all:{kind}") }
...
pub async fn main(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    Router::new()
        ...
        .post_async("/ingest", |req, ctx| async move {
            ...
            let limit: Option<usize> = req.url()?.query_pairs()
                .find(|(k, _)| k == "limit").and_then(|(_, v)| v.parse().ok());

            let mut resp = Fetch::Url("https://docs.ssccs.org/search.json".parse().unwrap())
                .send().await.map_err(|e| Error::RustError(format!("fetch: {e}")))?;
            let body = resp.text().await.map_err(|e| Error::RustError(format!("read: {e}")))?;
            ...
            for entry in batch {
                let id = if entry.objectID.is_empty() { format!("doc_{}", count) } else { entry.objectID.clone() };
                let tags: Vec<String> = if entry.crumbs.is_empty() { vec![entry.section.clone()] } else { entry.crumbs.clone() };

                let r2_key = content_key(&id);
                r2.put(&r2_key, entry.text.as_bytes().to_vec()).execute().await?;

                let fact = Fact {
                    id: id.clone(), origin: "docs.ssccs.org".into(), tags: tags.clone(),
                    title: entry.title.clone(), href: entry.href.clone(),
                    text_len: entry.text.len(), r2_key,
                    creator: "system".into(), created_at: now.clone(),
                };
                kv.put(&fact_key(&id), serde_json::to_string(&fact)?)?.execute().await?;
                for tag in &tags { if !tag.is_empty() { index_append(&kv, &tag_index_key(tag), &id).await?; } }
        ...
        .get_async("/facts", |req, ctx| async move {
            let kv = ctx.kv("FIH_KV")?;
            let tag: Option<String> = req.url()?.query_pairs()
                .find(|(k, _)| k == "tag").map(|(_, v)| v.to_string());
            let ids: Vec<String> = if let Some(ref t) = tag {
                kv.get(&tag_index_key(t)).text().await?
                    .and_then(|v| serde_json::from_str(&v).ok()).unwrap_or_default()
            } else {
                kv.get(&all_index("fact")).text().await?
                    .and_then(|v| serde_json::from_str(&v).ok()).unwrap_or_default()
            };
            let mut facts = Vec::new();
            for id in &ids {
                if let Some(raw) = kv.get(&fact_key(id)).text().await? {
                    if let Ok(f) = serde_json::from_str::<Fact>(&raw) { facts.push(f); }
                }
            }
        ...
        .get_async("/content/:id", |_req, ctx| async move {
            ...
            let fact: Fact = serde_json::from_str(&raw).map_err(|_| Error::RustError("not found".into()))?;
        ...
        .post_async("/intents", |mut req, ctx| async move {
            ...
            let intent = Intent {
                id: id.clone(), from_facts: body.from_facts, description: body.description,
                creator: body.creator, worker: None, to_fact_id: None,
                created_at: Some(Date::now().as_millis().to_string()), concluded_at: None,
            };
            kv.put(&intent_key(&id), serde_json::to_string(&intent)?)?.execute().await?;
        ...
        .post_async("/intents/:id/conclude", |mut req, ctx| async move {
            ...
            r2.put(&r2_key, body.result.as_bytes().to_vec()).execute().await?;

            let fact = Fact {
                id: fact_id.clone(), origin: "gateway".into(),
                tags: intent.from_facts.clone(),
                title: format!("Output of {}", id), href: String::new(),
                text_len: body.result.len(), r2_key,
                creator: intent.creator.clone(),
                created_at: Date::now().as_millis().to_string(),
            };
            kv.put(&fact_key(&fact_id), serde_json::to_string(&fact)?)?.execute().await?;
        ...
        .get_async("/intents", |_req, ctx| async move {
            let kv = ctx.kv("FIH_KV")?;
            let ids: Vec<String> = kv.get(&all_index("intent")).text().await?
                .and_then(|v| serde_json::from_str(&v).ok()).unwrap_or_default();
            let mut intents = Vec::new();
            for id in &ids {
                if let Some(raw) = kv.get(&intent_key(id)).text().await? {
                    if let Ok(i) = serde_json::from_str::<Intent>(&raw) { intents.push(i); }
                }
            }
        ...
        .get_async("/state", |_req, ctx| async move {
            ...
            Response::from_json(&serde_json::json!({"facts": facts, "intents": intents, "hints": hints}))
        })
        .run(req, env).await
}
...
async fn count_index(kv: &KvStore, kind: &str) -> u64 {
    match kv.get(&all_index(kind)).text().await {
        Ok(Some(v)) => serde_json::from_str::<Vec<String>>(&v).ok().map(|v| v.len() as u64).unwrap_or(0),
        _ => 0,
    }
}

fn uid() -> String { Date::now().as_millis().to_string() }
...
struct SearchEntry {
    #[serde(default)] objectID: String, #[serde(default)] href: String,
    #[serde(default)] title: String, #[serde(default)] section: String,
    #[serde(default)] text: String, #[serde(default)] crumbs: Vec<String>,
}

#[derive(Deserialize)]
struct IntentInput { id: Option<String>, from_facts: Vec<String>, description: String, creator: String }

#[derive(Deserialize)]
struct ClaimInput { agent: String }

#[derive(Deserialize)]
struct ConcludeInput { result: String }
