...
fn content_id_is_canonical() {
    ...
    assert_eq!(CoordId::from_string(&s), Some(id), "canonical string round-trips");
}
...
fn from_label_is_deterministic_and_canonical() {
    ...
    assert_eq!(CoordId::from_string(&s), Some(a));
}
...
fn resolve_accepts_canonical_and_label_equivalently() {
    ...
    assert_eq!(CoordId::resolve(label), derived, "label resolves to its derivation");
    assert_eq!(CoordId::resolve(&canonical), derived, "canonical form resolves identically");
    assert_eq!(CoordId::resolve(&canonical), CoordId::from_string(&canonical).unwrap());
}

#[test]
fn from_string_is_canonical_only() {
    let canonical = CoordId::new(42);
    assert_eq!(CoordId::from_string(&canonical.to_string()), Some(canonical));
    assert_eq!(CoordId::from_string("not-hangul-id"), None);
    assert_eq!(CoordId::from_string(""), None);
    assert_eq!(CoordId::from_string("가나다라마"), None, "wrong length is not canonical");
}
...
fn fact_new_is_content_addressed() {
    ...
    assert_eq!(f1.id, f2.id, "same content+origin+creator produces the same id");
