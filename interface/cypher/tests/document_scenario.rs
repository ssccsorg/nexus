...
use nex_fih::{Blackboard, BoardState, Content, CoordId, EvictCapable, Fact, Intent, StorageRead};
...
fn count_detector_facts(state: &BoardState, detector: &str, fact_type: &str) -> usize {
    state
        .facts
        .iter()
...
fn agent_resolve_contradictions(
    sched: &mut Scheduler<impl Blackboard + EvictCapable>,
    agent_name: &str,
    topic_filter: &str,
    conclusion: &str,
) {
...
fn scenario_full_document_lifecycle() {
    ...
    assert_eq!(
        nda_facts, 27,
        "Phase 4: new facts analyzed (SHA256-based IDs)"
    );
...
fn scenario_flush_then_evict() {
    ...
    // Second tick — no new facts (checkpoint stable)
    let r2 = sched.tick().expect("second tick");
    assert_eq!(r2, 0, "No new facts on second tick");
}
